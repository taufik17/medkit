...
import 'package:medkit/doctor/dokterPanel.dart';
import 'package:medkit/otherWidgetsAndScreen/backBtnAndImage.dart';
import 'package:toast/toast.dart';

class DokterLogin extends StatefulWidget {
  @override
  _DokterLoginState createState() => _DokterLoginState();
}
...
class _DokterLoginState extends State<DokterLogin> {
  ...
  Future<FirebaseUser> _signIn(BuildContext context) async {
    ...
    DokterDetails details = new DokterDetails(
  ...
  validateCNIC(String idNumber) {
    if(!(idNumber.length == 13) && idNumber.isNotEmpty) {
      return "No pegawai harus berjumlah 13";
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    getInfoAndLogin() {
      Firestore.instance
          .collection('InfoDokter')
          .document(_controllerName.text)
          .setData({
        'ID': _controllerCNIC.text,
      });
      _signIn(context)
          .then((FirebaseUser user) =>
          print('Log in dengan Gmail'))
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                            ...
                            onPressed: ()  {
                              setState(() {
                                ...
                                !validateName & !validateCNICVar ? getInfoAndLogin() :
                                    Toast.show("Bilah kosong ditemukan!", context, backgroundColor: Colors.red, backgroundRadius: 5, duration: Toast.LENGTH_LONG);
                        ...
                        WidgetAnimator(
                          Text(
                            'Anda akan ditanyai Pertanyaan tentang Kualifikasi Anda!',
...
class DokterDetails {
  ...
  DokterDetails(this.providerDetails, this.userName, this.photoUrl,
      this.userEmail, this.providerData);
}

class ProviderDetailDokter {
  ProviderDetailDokter(this.providerDetails);
